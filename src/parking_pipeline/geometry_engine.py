...
from .export_sqlite import MANIFEST_FILENAME, SQLITE_FILENAME, create_parking_database
...
def _print_timing_summary(
    *,
    ...
    log.info("   Timing:")
    log.info(
        f"     TCL intersections load: {_format_duration(timing.get('intersections_load', 0.0))}"
    )
    ...
    if warm > 0 or timing.get("intersection_warm_cache"):
        warm_label = (
            "(disk cache)" if timing.get("intersection_warm_cache") else _format_duration(warm)
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--geojson",
        action="store_true",
        default=os.environ.get("PARKING_EXPORT_GEOJSON", "").lower() in {"1", "true", "yes"},
        help="Also export final_parking_map.geojson (default: SQLite release artifact only)",
    )
    ...
    log.info(f"\n5. Exporting {len(results)} zones to SQLite ({SQLITE_FILENAME})...")
    ...
    if results:
        t0 = time.perf_counter()
        sqlite_out = data_path(SQLITE_FILENAME)
        manifest_out = data_path(MANIFEST_FILENAME)

        method_counts = Counter(str(row.get("curb_geometry_method") or "") for row in results)
        confidences = [
        ...
        # 1. Export SQLite database and release manifest (default release artifact)
        ...
        # 2. Optionally export GeoJSON FeatureCollection
        if args.geojson:
            ...
            with out_path.open("w", encoding="utf-8") as f:
                json.dump(geojson, f)
            log.info(f"Exported GeoJSON to '{out_path}'.")

        export_sec = time.perf_counter() - t0
        log.info(f"Done! SQLite release database written to '{sqlite_out}'.")
